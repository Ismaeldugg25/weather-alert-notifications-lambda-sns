...
# Install the 'requests' dependency into the build directory
resource "null_resource" "install_dependencies" {
  triggers = {
    requirements = md5(local_file.lambda_code.content)
}

  provisioner "local-exec" {
    command = "pip3 install requests -t ${path.module}/build --quiet --upgrade"
  }

  depends_on = [local_file.lambda_code]
}

# Create deployment package for Lambda (code + dependencies)
